...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const data = permisData[params.permis]
  if (!data) return { title: 'Inscription — Permisexpress.fr' }
  return {
    title: `Inscription ${data.title} — Permisexpress.fr`,
    description: data.description,
  }
}

export default function InscriptionPermisPage({ params }: Props) {
  const data = permisData[params.permis]
  if (!data) notFound()
    ...
    <main className="inscription-page">
      <div className="container inscription-page__container">
        ...
        <div className="product-breadcrumb" style={{ marginBottom: '32px' }}>
          ...
          {' / '}
          <span style={{ color: 'var(--blue)', fontWeight: 600 }}>Inscription — {data.title}</span>
        </div>
        ...
        <div className="inscription-page__header">
          ...
          <h1 style={{ color: '#fff', fontSize: '32px', fontWeight: 900, margin: '0 0 8px' }}>{data.title}</h1>
          <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: '15px', margin: '0 0 16px', lineHeight: 1.6 }}>{data.description}</p>
        ...
        <div className="inscription__layout">
          ...
          <div className="inscription__form-card">
            <InscriptionForm permis={data} slug={params.permis} />
          </div>
            ...
            <div style={{ background: 'var(--white)', borderRadius: 'var(--radius-lg)', padding: '24px', boxShadow: 'var(--shadow-sm)', border: '1px solid var(--border)' }}>
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {data.documents.map(doc => (
                  <li key={doc} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', fontSize: '13px', color: 'var(--text)', lineHeight: 1.5 }}>
